...
class Mglobal extends CI_Model{
    ...
    public function activitedAccount($code){
        $q = "SELECT full_name,email,id FROM tbl_users WHERE activated_key='".$code."'";
    ...
    public function getPost($slug){
        ...
        $res = $res->row();
        if ($res){
            $viewed = $res->viewed > 0 ? $res->viewed : 0 ;
            $viewed++;
            $q = "UPDATE tbl_post SET viewed=$viewed WHERE id=".$res->id;
            $this->db->query($q);
        }
        return $res; 
    }
    ...
    public function getBestPost(){
        $q = "SELECT * FROM tbl_post WHERE post_type='post' AND viewed > 1 ORDER by viewed DESC LIMIT 10";
        $res = $this->db->query($q);
        $res = $res->result();
    ...
    public function listComments($id){
        $q = "SELECT c.*,u.full_name FROM tbl_comments c "; 
        $q.= "LEFT JOIN tbl_users u ON u.id = c.user_id WHERE c.post_id='".$id."' order by id DESC LIMIT 10 ";
        $res = $this->db->query($q);
        $res = $res->result();
    ...
    public function checkEmail($email){
        $q = "SELECT activated_key,status,email,username FROM tbl_users WHERE email='".$email."' OR username='".$email."'";
        $res = $this->db->query($q);
        return $res->row();
    }
