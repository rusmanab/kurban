...
class MY_Model extends CI_Model{
    ...
    public function getData($row=false){
        if (!empty($this->param)){            
    		...
    		if( ( !empty($config['limit']) ) ){
    		    if (empty($config['star']) || !isset($config['star']) ){
    				$star='0';
    			}else{
    				$star=$config['star'];
    			}
    			$this->db->limit($config['limit'],$star);	
    		}
		...
		if ( isset($request['search']) && $request['search']['value'] != '' ) {
			...
			for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
				...
				if ( $requestColumn['searchable'] == 'true' ) {
					...
					$globalSearch[] = $mySuffix."`".$column['db']."` LIKE '%".$str."%'";
				}
			}
		}
    ...
    public function setMaxSizeUpload($inKb){
        $this->_max_size = $inKb;
    }
    ...
    public function uploadImage($directory, $fileinput="userfile", $thumbs = true,$width=120,$height=0, $unlink = false ){        
        ...
        if (isset($this->mimeType)){
            if (!empty($this->mimeType)){
                $allowed_types.=$this->mimeType;
            }
        }
        
        
        $setting = array(
				"upload_path"	=> "..".DIRECTORY_SEPARATOR."assets".DIRECTORY_SEPARATOR."images".DIRECTORY_SEPARATOR.$directory,
        ...
        if($this->upload->do_upload($fileinput)){
              ...
              $dir      = "..".DIRECTORY_SEPARATOR."assets".DIRECTORY_SEPARATOR."images".DIRECTORY_SEPARATOR.$directory;
              ...
              if ($thumbs){
                  ...
                  $resize->resizeTo($fWidth,$fHeight,$mode );
