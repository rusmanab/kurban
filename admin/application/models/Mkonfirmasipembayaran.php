<?php
class Mkonfirmasipembayaran extends MY_Model{
    protected static $table	='tbl_konfrmasi_pembayaran';
    ...
    public function getConfirmation($noorder){
        $q = "SELECT p.*, s.nama_status FROM ".self::$table." p LEFT JOIN tbl_status s ON s.id = p.status_id 
                    WHERE p.no_order='".$noorder."'";
        $res = $this->db->query($q);
